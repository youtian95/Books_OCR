...
8.2.1 除圆管截面外，弯矩作用在对称轴平面内的实腹式压弯构件，弯矩作用平面内稳定性应按式（8.2.1-3）计算；对于本标准表8.1.1第3、4项中的单轴对称压弯构件，当弯矩作用在非对称平面内且使骤缘受压时，除应按式（8.2.1-1）计算外，尚应按式（8.2.1-4）计算；当框架内力采用二阶弹性分析时，柱弯矩由无侧移弯矩和放大的侧移弯矩组成，此时可对两部分弯矩分别乘以无侧移柱和有侧移柱的等效弯矩系数。
 ...
 $$ N_{\mathrm{Ex}}^{\prime}=\pi^{2}EA\Big/\Big(1.1\lambda^{2}_{\mathrm{x}}\Big) $$ 
...
式中： $ \sum N_{bi}, \sum N_{0i} $  ——分别是第 i 层层间所有框架柱用无侧移框架和有侧移框架柱计算长度系数算得的轴压杆稳定承载力之和（N）；
